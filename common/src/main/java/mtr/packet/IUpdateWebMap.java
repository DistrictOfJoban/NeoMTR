	...
	static <T extends AreaBase> void iterateAreas(Set<T> areas, AreaCallback areaCallback) {
		areas.forEach(area -> {
			...
			if (corner1 != null && corner2 != null && areaPos != null) {
				areaCallback.areaCallback(area.id + "_" + System.currentTimeMillis(), IGui.formatMTRLanguageName(area.name), new Color(area.color), corner1.getA(), corner1.getB(), corner2.getA(), corner2.getB(), areaPos.getX(), areaPos.getZ());
			}
		});
	}
