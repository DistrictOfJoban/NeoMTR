...
public class SideDashboardList implements IGui {
    ...
    public <T> SideDashboardList(BiConsumer<NameColorDataBase, Integer> onFind, BiConsumer<NameColorDataBase, Integer> onDrawArea, BiConsumer<NameColorDataBase, Integer> onEdit, Runnable onSort, BiConsumer<NameColorDataBase, Integer> onAdd, BiConsumer<NameColorDataBase, Integer> onDelete, Supplier<List<T>> getList, Supplier<String> getSearch, Consumer<String> setSearch) {
        this(onFind, onDrawArea, onEdit, onSort, onAdd, onDelete, getList, getSearch, setSearch, true);
    }

    public <T> SideDashboardList(BiConsumer<NameColorDataBase, Integer> onFind, BiConsumer<NameColorDataBase, Integer> onDrawArea, BiConsumer<NameColorDataBase, Integer> onEdit, Runnable onSort, BiConsumer<NameColorDataBase, Integer> onAdd, BiConsumer<NameColorDataBase, Integer> onDelete, Supplier<List<T>> getList, Supplier<String> getSearch, Consumer<String> setSearch, boolean playSound) {
    ...
    public void init(Consumer<AbstractWidget> addDrawableChild) {
        IDrawing.setPositionAndWidth(buttonPrevPage, x, y + height - SQUARE_SIZE, SQUARE_SIZE);
        IDrawing.setPositionAndWidth(buttonNextPage, x + width - SQUARE_SIZE, y + height - SQUARE_SIZE, SQUARE_SIZE);
        IDrawing.setPositionAndWidth(textFieldSearch, x + (TEXT_FIELD_PADDING / 2), y + TEXT_FIELD_PADDING / 2, width - TEXT_FIELD_PADDING);
    ...
    public void render(GuiGraphics guiGraphics, Font font) {
        renderPageNumber(guiGraphics, font);
        ...
        for (int i = 0; i < itemsToShow; i++) {
            if (i + itemsToShow * page < dataFiltered.size()) {
                ...
                guiGraphics.drawString(font, drawString, 0, y + drawY, ARGB_WHITE);
                guiGraphics.pose().popPose();
            }
        }
    }

    private void renderPageNumber(GuiGraphics guiGraphics, Font font) {
        Component pageComponent = Component.literal(String.format("%s/%s", page + 1, totalPages));
        guiGraphics.drawCenteredString(font, pageComponent, x + (width / 2), y + height - TEXT_HEIGHT - TEXT_PADDING, ARGB_WHITE);
    }
    ...
    private int itemsToShow() {
        return (height - TOP_OFFSET - SQUARE_SIZE) / SQUARE_SIZE;
    }
}
